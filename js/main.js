...
function popApproval(kart, opts = {}) {
  ...
  if (kart.isPlayer) {
    ...
    Sound.spatialTone(kart.x, kart.y, 200, 0.14, "sawtooth", 0.12, 90);
  }
  if (game.particles) {
...
const Sound = {
  ...
  musicVolume: normalizeVolume(savedSettings.musicVolume, 0.8),
  sfxVolume: normalizeVolume(savedSettings.sfxVolume, 1.0),
  musicBaseGain: 0.08,
  musicDuckGain: null,
  ...
  setMuted(m) {
    ...
    saveGameSettings({ muted: this.muted });
  },
  setMusicVolume(v, opts = {}) {
    ...
    if (opts.persist !== false) saveGameSettings({ musicVolume: this.musicVolume });
  },
  setSfxVolume(v, opts = {}) {
    this.sfxVolume = normalizeVolume(v, 1.0);
    if (this.sfxGain) this.sfxGain.gain.value = this.sfxVolume;
    if (opts.persist !== false) saveGameSettings({ sfxVolume: this.sfxVolume });
  },
  ...
  _spatialParams(x, y, vol) {
    if (!game.player) return { pan: 0, scaledVol: vol };
  ...
  trackIdx: Number.isInteger(savedSettings.musicTrack) ? savedSettings.musicTrack : null,
  toastTimeout: null,
...
function integrateKartVertical(kart, dt) {
  ...
  if (z <= 0) {
    ...
    kart.z = 0;
    kart.vz = 0;
    if (wasAir && kart.isPlayer && game.particles) {
      spawnRampLandingFx(kart);
...
function tryApprovalRam(att, def, dirx, diry) {
  ...
  if (def.isPlayer || att.isPlayer) { Sound.crash(); game.shake = Math.max(game.shake, 8); }
